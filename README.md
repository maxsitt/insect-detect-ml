...
script can be used to filter out corrupted .jpg images in the data folder (camera trap output).
These can sometimes be generated by the automated monitoring script and will cause an error
while running the classification script. Additionally the mean/min/max image width and image
height are calculated, which can be helpful e.g. for training a custom classification model.
